...
export default function Strip({ curSlide, onSelect }) {
    ...
    useEffect(() => {
        // Center the active strip
        const strip = scrollRef.current;
        if (!strip) return;
        const activeEl = strip.children[curSlide];
        if (activeEl) {
            const target = activeEl.offsetLeft - strip.clientWidth / 2 + activeEl.offsetWidth / 2;
    ...
    return (
        <div className="fixed top-[52px] left-0 right-0 h-[44px] bg-surface border-b border-border z-[200] overflow-hidden">
            ...
            <div ref={scrollRef} className="w-full h-full overflow-x-auto overflow-y-hidden scrollbar-none flex items-center">
                <div className="flex items-stretch h-full flex-shrink-0 min-w-full">
                    {stocks.map((s, i) => {
                        const p = pct(s);
                        const c = cls(s);
